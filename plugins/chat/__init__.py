...
"interfaces": ["gtk",], "description": "Delete the chat log"}
    ...
    def request(self, url, certhash, action, arguments, traverseserveraddr=None): #, timestamp=create_timestamp()): #, _traversefunc()
        ...
        #sock, _cert, _hash = 
        #, timestamp=timestamp
        return self.resources("plugin")(url, "chat", reqstring.format(action=action, privstate=privstate, other=arguments), forcehash=certhash, traverseserveraddr=traverseserveraddr)
    ...
    def receive(self, action, _socket, _cert, certhash):
        ...
        with chatlock[certhash]:
            if action == "send_text":
                ...
                if "gtk" in self.interfaces and gtk is not None:
